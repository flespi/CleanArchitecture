...
export interface ITodoItemsClient {
    ...
    readTodoItem(id: number): Observable<TodoItemDto>;
    updateTodoItem(id: number, concurrencyToken: string, command: UpdateTodoItemCommand): Observable<void>;
    deleteTodoItem(id: number): Observable<void>;
    updateTodoItemDetail(id: number, concurrencyToken: string, command: UpdateTodoItemDetailCommand): Observable<void>;
}
...
export class TodoItemsClient implements ITodoItemsClient {
    ...
    readTodoItem(id: number): Observable<TodoItemDto> {
        ...
        return this.http.request("get", url_, options_).pipe(_observableMergeMap((response_ : any) => {
            return this.processReadTodoItem(response_);
        })).pipe(_observableCatch((response_: any) => {
            if (response_ instanceof HttpResponseBase) {
                try {
                    return this.processReadTodoItem(response_ as any);
                } catch (e) {
    ...
    protected processReadTodoItem(response: HttpResponseBase): Observable<TodoItemDto> {
        const status = response.status;
        const responseBlob =
            response instanceof HttpResponse ? response.body :
            (response as any).error instanceof Blob ? (response as any).error : undefined;
...
export interface ITodoListsClient {
    ...
    updateTodoList(id: number, concurrencyToken: string, command: UpdateTodoListCommand): Observable<void>;
