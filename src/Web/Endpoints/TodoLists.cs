...
public class TodoLists : EndpointGroupBase
{
    ...
    public async Task<Created<int>> CreateTodoList(ISender sender, CreateTodoListDto data)
    {
        var id = await sender.Send(new CreateTodoListCommand { Data = data });

        return TypedResults.Created($"/{nameof(TodoLists)}/{id}", id);
    }

    public async Task<Results<NoContent, BadRequest>> UpdateTodoList(ISender sender, int id, UpdateTodoListDto data)
    {        
        await sender.Send(new UpdateTodoListCommand { Id = id, Data = data });
